...
public class DBDataManager implements IDataManager {
    ...
    @Override
    public void loadAll() {
        FileConfiguration fc = RegisterSmart.instance.getConfig();
        ...
        if (port == null || username == null || db_name == null || password == null) {
            RegisterSmart.dbError = true;
            RegisterSmart.instance.getLogger().warning("数据库配置不完全，将改用备用存储方式。");
            return;
        }
        ...
        try {
            ...
            statement.close();
            connection.close();
            RegisterSmart.instance.getLogger().info("成功与数据库建立连接！");
    ...
    private void putError(Exception e) {
        RegisterSmart.dbError = true;
        RegisterSmart.instance.getLogger().warning("数据库操作失败，将改用备用存储方式。");
        e.printStackTrace();
    }
    ...
    public Date getLastLoginTime(UUID id) {
        try {
            ...
            try {
                ...
                RegisterSmart.instance.getLogger().warning("这不可能！不可能出现这个错误！日期的读取失败了？");
                e2.printStackTrace();
                return new Date();
            }
